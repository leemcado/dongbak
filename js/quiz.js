...
/* 삽화는 왼쪽, 선지는 오른쪽에 세로로. 가운데는 문짝(x 335~625)이
   드러나도록 비워 둔다. 좌우 여백을 68 / 62 로 맞춰 한쪽으로 쏠리지 않게 한다. */
box(200,344,264,264,28,'illust');
for(var i=0;i<4;i++)box(748,196+i*96,300,80,24,'o',i);
/* 세로로 늘어선 선지는 왼끝을 맞춰야 목록으로 읽힌다. 가운데 정렬이면
   글자 길이마다 시작점이 달라져 오른쪽으로 쏠려 보인다. */
for(var i=0;i<B.length;i++)if(B[i].role==='o')B[i].align='left';}
